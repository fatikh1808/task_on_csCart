...
function fn_newsletters_get_recipients(array $params)
{
    ...
    if (!empty($params['mailing_lists'])) {
        $list_recipients = db_get_array(
            'SELECT 0 as user_id, subscribers.email, subscribers.lang_code, mailing_lists.list_id  FROM ?:subscribers AS subscribers'
    ...
    if (!empty($params['users'])) {
        ...
        $user_recipients = db_get_array('SELECT users.user_id, users.email, users.lang_code, NULL as list_id FROM ?:users AS users WHERE users.user_id IN (?n)', $users);
    }
    ...
    if (!empty($params['abandoned_days'])) {
        ...
        $abandoned_recipients = db_get_array(
            'SELECT users.user_id, users.email, users.lang_code, NULL as list_id FROM ?:users AS users'
...
function fn_newsletters_add_batch_recipients(array $recipient_list)
{
    ...
    foreach ($recipient_list_chunks as $recipient_list) {
        db_query('INSERT INTO ?:newsletter_batch_recipients ?m', $recipient_list);
    }
}
