...
function fn_get_companies($params, &$auth, $items_per_page = 0, $lang_code = CART_LANGUAGE)
{
    ...
    if (!empty($params['extend'])) {
        ...
        if (!empty($params['extend']['products_count'])) {
            $companies_products_count = fn_get_companies_active_products_count();
        }
 ...
 *
 * @param array $company_ids Allows to limit query by specified companies, all allowed company will be get by default
 *
 * @return array Companies and their products count list
 */
 ...
 */
function fn_get_company_root_admin_user_id($company_id)
{
    if (empty($company_id)) {
        return 0;
    }
