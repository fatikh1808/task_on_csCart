...
<div class="table-responsive-wrapper">
    ...
    <thead>
    <tr>
        <th width="1%" class="center {$no_hide_input} mobile-hide">
        ...
        <th width="12%"><a class="cm-ajax" href="{"`$c_url`&sort_by=last_login&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__("last_login")}{if $search.sort_by == "last_login"}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
        <th>{__("phone")}</th>
        {if !$search.user_type}
            <th>{__("type")}</th>
        {/if}
        {if $smarty.request.user_type == "UserTypes::CUSTOMER"|enum && $can_view_orders}
            <th width="15%">{__("orders")}</th>
        {/if}
    ...
    {/if}
        <td class="center {$no_hide_input} mobile-hide">
            <input type="checkbox" name="user_ids[]" value="{$user.user_id}" class="cm-item" /></td>
